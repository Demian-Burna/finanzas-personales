...
export function DataTab() {
  ...
  function handleDeleteAll() {
    if (confirmation !== 'CONFIRMAR') {
      toast.error('Escribí CONFIRMAR para proceder')
      return
    }
    startDelete(async () => {
      const res = await deleteAllDataAction(confirmation)
      ...
      <section className="space-y-4 rounded-xl border border-destructive/30 bg-destructive/5 p-5">
        ...
        <div className="space-y-2">
          <Label htmlFor="confirm-delete" className="text-xs">
            Escribí <span className="font-mono font-bold">CONFIRMAR</span> para habilitar el botón
          </Label>
          <Input
            id="confirm-delete"
            value={confirmation}
        ...
        <Button
          variant="destructive"
          size="sm"
          disabled={confirmation !== 'CONFIRMAR' || isDeleting}
          onClick={handleDeleteAll}
        >
          {isDeleting ? 'Eliminando...' : 'Eliminar todos mis datos'}
        </Button>
      </section>
    </div>
  )
